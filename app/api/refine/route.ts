...
// 아이가 작품 앞에서 한 말을 그대로 받아 적은 원문(1인칭 반말·외침)인지에 따라
// '교정'과 '도슨트 마무리' 중 어느 쪽으로 다듬을지 프롬프트가 갈린다.
...
- 문장 수와 길이를 원문 그대로 유지합니다. 요약하지도, 늘리지도 않습니다. (아래 '도슨트 마무리'만 예외)
...
도슨트 마무리 — 원문이 아이가 작품 앞에서 한 말을 그대로 받아 적은 것일 때만:
- 원문 전체가 아이의 말 그 자체이면(1인칭 반말·외침, 예: "이건 화산폭발이 일어난 거야"), 그 말을 따옴표로 그대로 살리고, 도슨트가 관람객 곁에서 소개하듯 그 말이 무엇을 표현한 것인지 한 문장으로 마무리합니다.
  예: 원문 "이건 화산폭발이 일어난거야" → "이건 화산폭발이 일어난 거야." — 화산이 폭발하는 순간을 표현한 작품이에요.
- 마무리 문장은 아이의 말에 이미 담긴 내용만 풀어 씁니다. 원문에 없는 재료·색·이야기·감정을 지어내지 않습니다.
- 부모가 이미 서술형으로 쓴 글에는 이 마무리를 붙이지 않습니다. 그때는 위의 교정만 합니다.
